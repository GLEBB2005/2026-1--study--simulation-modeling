# # Сканиование коэффициента заразности B
#
# Исследует, как изменение B влияет на пик эпидемии и смертность.
using DrWatson
...
# Диапазон значений beta и seed для повторов 
beta_range = 0.1:0.1:1.0
seeds = [42, 43, 44]

# Генерация комбинация параметров
params_list = []
for b in beta_range
    for s in seeds
...
# График
plot(grouped.beta, grouped.mean_peak, label = "Пик эпидемии", xlabel = "Коэффициент заразности β", 
